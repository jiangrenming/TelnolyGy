...
public class VoiceSpeakers {


    private static VoiceSpeakers sInstance;
    ...
    private  static  String mTransType;


    private VoiceSpeakers() {
        service = Executors.newCachedThreadPool();

    }

    private static synchronized VoiceSpeakers getInstance() {
        if (sInstance == null) {
            sInstance = new VoiceSpeakers();
        }
        return sInstance;
    }
    ...
    public  static class  Builder{
        ...
        VoiceSpeakers mVoice ;
        public Builder(Context context){
            mContext=  context;
            mVoice = VoiceSpeakers.getInstance();
        }
        /**
         * 设置音频的格式配置
         * @param audioParams
     ...
     * @param transType
     */
    public  static void setTransType(String transType){
                    ...
                    synchronized (this){
                        try {
                            if (voiceType == 0){            //AudioTrack类实现读取语言
                                ...
                                if (mResId != 0){        //外界直接传入assets或raw中的音频文件
